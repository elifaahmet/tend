...
import {
  StickyDisclaimer,
  StickyDisclaimerProps,
  injectStickyFooter,
} from '@tend/shared/design/ui-kit';

export default {
  component: StickyDisclaimer,
  title: 'StickyDisclaimer',
} as Meta;
...
function removeStickyFromDOM() {
  const el: Element = document.getElementsByClassName('tend-sticky-footer')[0];
  if (el && typeof el.remove === 'function') {
    el.remove();
  }
}

const DefaultSticky: React.FC = () => (
  <StyledDiv>
    <h1>This Footer is sticky and dynamically injected...</h1>
  </StyledDiv>
);

const Template: Story<StickyDisclaimerProps> = (args) => {
  useEffect(() => {
    return removeStickyFromDOM;
  }, []);
  ...
  return (
    <StyledWrapper>
      ...
      <LoremIpsum />
      <LoremIpsum />
      <StickyDisclaimer {...args} />
    </StyledWrapper>
  );
};

export const asChildren = Template.bind({});
asChildren.args = {
  children: <DefaultSticky />,
};

export const AsScript: React.FC = () => {
  useEffect(() => {
    injectStickyFooter(renderToString(<DefaultSticky />));
    return removeStickyFromDOM;
  }, []);
